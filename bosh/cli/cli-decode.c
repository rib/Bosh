...
static void help_all (GIOChannel *stream);
...
void
bosh_command_set_callback (struct cmd_list_element *cmd, cmd_cfunc_ftype *cfunc)
{
  if (cfunc == NULL)
    cmd->func = NULL;
  else
...
void
bosh_command_set_setshow_callback (struct cmd_list_element *cmd, cmd_sfunc_ftype *sfunc)
{
  if (sfunc == NULL)
    cmd->func = NULL;
  else
...
void
bosh_command_set_context (struct cmd_list_element *cmd, void *context)
{
  cmd->context = context;
}

void *
bosh_command_get_context (struct cmd_list_element *cmd)
{
  return cmd->context;
}
...
void
bosh_command_set_completer (struct cmd_list_element *cmd,
                       ...
                       char *doc)
{
  ...
  c->allow_unknown = 0;
  c->abbrev_flag = 0;
  bosh_command_set_completer (c, make_symbol_completion_list);
  c->type = not_set_cmd;
  c->var = NULL;
...
struct cmd_list_element *
bosh_command_list_add_alias (struct cmd_list_element **list,
                             char *name,
                             char *oldname,
                             enum command_class class,
                             int abbrev_flag)
...
/* Like bosh_command_list_add but adds an element for a command prefix:
   a name that should be followed by a subcommand to be looked up
   in another command list.  PREFIXLIST should be the address
   of the variable containing that list.  */

struct cmd_list_element *
bosh_command_list_add_prefix (struct cmd_list_element **list,
                              char *name,
                              enum command_class class,
                              void (*fun) (char *, int),
                              char *doc,
                              ...
                              int allow_unknown)
{
  struct cmd_list_element *c = bosh_command_list_add (list, name, class, fun, doc);
...
/* Like bosh_command_list_add_prefix but sets the abbrev_flag on the new command. */

struct cmd_list_element *
bosh_command_list_add_abbreviated_prefix (struct cmd_list_element **list,
...
/* Add element named NAME to command list LIST (the list for set/show
   ...
   CLASS is as in bosh_command_list_add.
   VAR_TYPE is the kind of thing we are setting.
   VAR is address of the variable being controlled by this command.
   DOC is the documentation string.  */

static struct cmd_list_element *
add_set_or_show_cmd (struct cmd_list_element **list,
                     char *name,
                     enum cmd_types type,
                     enum command_class class,
                     var_types var_type,
                     ...
                     char *doc)
{
  struct cmd_list_element *c = bosh_command_list_add (list, name, class, NULL, doc);
  g_assert (type == set_cmd || type == show_cmd);
  c->type = type;
  c->var_type = var_type;
  c->var = var;
  /* This needs to be something besides NULL so that this isn't
     treated as a help class.  */
  bosh_command_set_setshow_callback (c, empty_sfunc);
  return c;
}

/* Add element named NAME to both the command SET_LIST and SHOW_LIST.
   CLASS is as in bosh_command_list_add.  VAR_TYPE is the kind of thing we are
                      ...
                      struct cmd_list_element **show_result)
{
  ...
  if (help_doc != NULL)
    {
      full_set_doc = g_strdup_printf ("%s\n%s", set_doc, help_doc);
      full_show_doc = g_strdup_printf ("%s\n%s", show_doc, help_doc);
    }
  else
    {
      full_set_doc = g_strdup (set_doc);
      full_show_doc = g_strdup (show_doc);
    }
  set = add_set_or_show_cmd (set_list, name, set_cmd, class, var_type, var,
                             full_set_doc);
  if (set_func != NULL)
    bosh_command_set_setshow_callback (set, set_func);
  show = add_set_or_show_cmd (show_list, name, show_cmd, class, var_type, var,
                              full_show_doc);
...
{
  return add_set_or_show_cmd (list, name, set_cmd, class, var_type, var, doc);
}

/* Add element named NAME to command list LIST (the list for set or
   some sublist thereof).  CLASS is as in bosh_command_list_add.  ENUMLIST is a list
...
/* Add an auto-boolean command named NAME to both the set and show
   command list lists.  CLASS is as in bosh_command_list_add.  VAR is address of the
...
/* Add element named NAME to both the set and show command LISTs (the
   ...
   bosh_command_list_add.  VAR is address of the variable which will contain the
                          ...
                          struct cmd_list_element **show_list)
{
  ...
  add_setshow_cmd_full (name, class, var_filename, var,
                        ...
                        set_list, show_list,
                        &set_result, NULL);
  bosh_command_set_completer (set_result, filename_completer);
}
...
struct cmd_list_element *
bosh_add_info_command (char *name, void (*fun) (char *, int), char *doc)
{
  return bosh_command_list_add (&infolist, name, no_class, fun, doc);
}

/* Add an alias to the list of info subcommands.  */

struct cmd_list_element *
bosh_add_info_alias_command (char *name, char *oldname, int abbrev_flag)
{
  return bosh_command_list_add_alias (&infolist, name, oldname, 0, abbrev_flag);
}

/* Add an element to the list of commands.  */

struct cmd_list_element *
bosh_add_command (char *name, enum command_class class, void (*fun) (char *, int),
         char *doc)
{
  return bosh_command_list_add (&cmdlist, name, class, fun, doc);
}

/* Add an alias or abbreviation command to the list of commands.  */

struct cmd_list_element *
bosh_add_command_alias (char *name, char *oldname, enum command_class class,
               int abbrev_flag)
{
  return bosh_command_list_add_alias (&cmdlist, name, oldname, class, abbrev_flag);
}

/* Recursively walk the commandlist structures, and print out the
   ...
   */
void
apropos_cmd (GIOChannel *stream,
             struct cmd_list_element *commandlist,
             GRegex *regex, char *prefix)
{
  struct cmd_list_element *c;
  gboolean matched=FALSE; /*Needed to avoid double printing*/
  ...
  for (c=commandlist;c;c=c->next)
    {
      if (c->name != NULL)
        {
          ...
          matched=g_regex_match (regex, c->name, 0, &match_info);
          if (matched)
            {
      ...
      if (c->doc != NULL && !matched)
        {
          GMatchInfo *match_info;
          /* Try to match against documentation */
          if (g_regex_match (regex, c->doc, 0, &match_info))
            {
              g_match_info_free (match_info);
              print_help_for_command (c, prefix,
                                      0 /* don't recurse */, stream);
...
void
help_cmd (char *command, GIOChannel *stream)
{
  struct cmd_list_element *c;
...
help_list (struct cmd_list_element *list, char *cmdtype,
           enum command_class class, GIOChannel *stream)
{
  int len;
  ...
  if (class == all_classes)
    {
      fprintf_filtered (stream,
                        "\n"
                        "Type \"help%s\" followed by a class name "
                        "for a list of commands in ",
                        cmdtype1);
      wrap_here ("");
...
void
print_doc_line (GIOChannel *stream, char *str)
{
  ...
  if (!line_buffer)
    {
      line_size = 80;
      line_buffer = (char *) g_malloc (line_size);
    }
...
help_cmd_list (struct cmd_list_element *list, enum command_class class,
               char *prefix, int recurse, GIOChannel *stream)
{
  struct cmd_list_element *c;
...
complete_on_cmdlist (struct cmd_list_element *list, char *text, char *word)
{
  ...
  sizeof_matchlist = 10;
  matchlist = (char **) g_malloc (sizeof_matchlist * sizeof (char *));
  matches = 0;
    ...
    if (!strncmp (ptr->name, text, textlen)
            ...
            || ptr->prefixlist))
      {
        if (matches == sizeof_matchlist)
          {
            sizeof_matchlist *= 2;
            matchlist = (char **) g_realloc ((char *) matchlist,
                                             (sizeof_matchlist
                                              * sizeof (char *)));
          }

        matchlist[matches] = (char *)
          g_malloc (strlen (word) + strlen (ptr->name) + 1);
        if (word == text)
  ...
  if (matches == 0)
    ...
    {
      matchlist = (char **) g_realloc ((char *) matchlist, ((matches + 1)
                                                            * sizeof (char *)));
    ...
    if (strncmp (name, text, textlen) == 0)
      {
        ...
        matchlist[matches] = (char *)
          g_malloc (strlen (word) + strlen (name) + 1);
        if (word == text)
...
int
bosh_command_has_callback (struct cmd_list_element *cmd)
{
...
void
bosh_command_call (struct cmd_list_element *cmd, char *args, int from_tty)
{
  if (bosh_command_has_callback (cmd))
    (*cmd->func) (cmd, args, from_tty);
  else
    g_print (_("Invalid command"));
}
